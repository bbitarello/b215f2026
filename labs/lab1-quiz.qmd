...
filters:
  - webr
webr:
  packages: ['learnr', 'gradethis', 'knitr', 'tidyverse']
...
### exercise 2
Now create another vector, b, containing the numbers 2,9, and 8 in that order

```{r sim2, exercise = TRUE, allow_skip = FALSE}
        # assign a vector with 2, 9 and 8, in that order, to b
```
...
Now, test if vector a is equal to vector b
...
Finally, multiply vectors a and b
...
#| context: setup
learnr::quiz(
    learnr::question("In the example above, what would R tell you when you ask it if a equals b?",
  ...
  learnr::question("In the example above, what does multiplying a and b return?",
...
```{webr-r}
learnr::quiz(caption = "Quiz - Install R",
  learnr::question("Is R free to download and use?",
  ...
  ,
    learnr::question("What is the RStudio IDE?",
  ...
  learnr::question("Is the RStudio IDE free to download and use?",
...
gradethis::grade_result(
  gradethis::pass_if(~identical(.result, 255))
)
...
```{r ex2-check}
gradethis::grade_result(
  gradethis::pass_if(~identical(.result, 2197))
)  
```  
...
```{r ex3-check}
gradethis::grade_result(
  gradethis::pass_if(~identical(.result,  log(14)))
)  
```  
...
```{r ex4-check}
gradethis::grade_result(
  gradethis::pass_if(~identical(.result, 2))
)  
```  
...
gradethis::grade_result(
  #pass_if(~any(((9-0.5)<.result)==TRUE)&any(((9+0.5)>.result)==TRUE)
    ...
    gradethis::pass_if(~identical(.result, 9))
)  
```  
